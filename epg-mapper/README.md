...
Open `index.html` in Chrome, or publish the `epg-mapper` folder with GitHub Pages. Load `simple_channel_list.txt`, then load the current IPTV-EPG `.xml.gz` file or use the URL field. Select a provider channel, choose the matching IPTV-EPG channel ID, and select **Add mapping**. The **Download mappings.csv** button creates a file ready to review and commit.
