...
v1.4.6 notes:
- Limits Multiview presets to five and hides Save Preset at the limit.
- Shows preset confirmations in a temporary popup and increases Multiview tile size.
- Moves Multiview controls beside Refresh with consistent button sizing and spacing.
...
- Settings includes a public 5 MB internet download speed test with response time.

v1.4.3 notes:
- Multiview now detects stalled playback even when the player remains in a ready state and automatically retries the affected tile with throttled recovery.

v1.4.2 notes:
- Adds automatic multiview recovery after sustained tile buffering.
- Bounds per-tile buffering to reduce memory pressure when multiple streams are active.
...
- Adds DVR scheduling and recording management UI.
